...
%macro heavyDrinking(out=heavyDrinking, cchs_data=, demographics=, adultsOnly=true);
%local out cchs_data demographics adultsOnly;  %validate(data=&cchs_data,include=&demographics) data &out; set &cchs_data.(keep=ont_id alc_020 dhh_age &demographics); select(alc_020); when(3,4,5,6) heavyDrinking = 1; when(1,2,96) heavyDrinking = 0; otherwise heavyDrinking = .; end; 
%if %upcase(&adultsOnly) = TRUE %then %do; if dhh_age < 19 then do; heavyDrinking = .; exclude = 1; end;
%end;
...
%if %length(&data)=0 %then %do; %let _varflag=E; %let _varmsg= A dataset name is required for parameter OUT!; %goto err_exit;
%end;
%else %do; %let _dsid=%sysfunc(exist(&data,data));  %if &_dsid=0 %then %do; %let _varflag=E; %let _varmsg= The dataset &data does not exist!; %goto err_exit; %end;
%end; 

%if %length(&include) ne 0 %then %do; %let _dsid=%sysfunc(open(&data));  %local count this_var; %let count=1; %let this_var = %qscan(&include,&count,%str( )); %do %while(&this_var ne); %let _varnum=%sysfunc(varnum(&_dsid,&this_var)); %if &_varnum=0 %then %do;  %let _varflag=E; %let _varmsg= The variable &this_var does not exits in the dataset &data.!; %goto err_exit; %end; %let count = %eval(&count+1); %let this_var = %qscan(&demographics,&count,%str( )); %end;
%end;
%goto clean_exit; %err_exit:  %put ====================================================; %put The following error occurred while running the macro:; %put &_varmsg; %put; %put Please review input and try again.; %put; %put The macro will stop execution.; %put ====================================================;  %abort;
%clean_exit: %put ====================================================; %put No invalid parameters were detected.; %put The macro will continue dummy coding.; %put ====================================================; %mend validate;